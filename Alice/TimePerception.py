...
class QuantumTaskScheduler:
    ...
    async def schedule_task(self, task: Dict) -> Dict:
        ...
        tasks = self.scheduled_tasks + [task]
        optimized_tasks = await self.quantum_time.optimize_task_schedule(tasks)
        
        # Update schedule with optimized times
        self.scheduled_tasks = optimized_tasks
        return optimized_tasks[-1]

    async def get_next_task(self) -> Optional[Dict]:
        """Get next task based on quantum-optimized schedule"""
        if not self.scheduled_tasks:
            return None
            
        # Sort by scheduled time
        self.scheduled_tasks.sort(key=lambda x: x.get('scheduled_time', float('inf')))
